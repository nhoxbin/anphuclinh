...
var mstkmx = "undefined" == typeof max_token_msg ? "You can purchase the maximum token amount per contribution." : max_token_msg,
    mstkmn = "undefined" == typeof min_token_msg ? "Enter the minimum token amount and select currency!" : min_token_msg,
    mswng = "undefined" == typeof msg_wrong ? "Something is Wrong!" : msg_wrong,
    msasr = "undefined" == typeof msg_sure ? "Are you sure?" : msg_sure,
    msdlg = "undefined" == typeof msg_delete_log ? "Once Delete, You will not get back this log in future!" : msg_delete_log,
    msodr = "undefined" == typeof msg_cancel_order ? "Do you really cancel your order?" : msg_cancel_order,
    msups = "undefined" == typeof msg_unable_process ? "Unable process request!" : msg_unable_process,
...
function token_alert(a, b, c = "") {
    ...
    return "icon" === c ? void a.find(".note-icon").html("<i class=\"fas fa-" + b + "\"></i>") : "token" === c ? void a.find(".min-token").text(b) : "amount" === c ? void a.find(".min-amount").text(b) : "text" === c ? (a.find(".note-text-alert").html(b), void a.find(".note-text-alert").addClass("text-danger")) : void a.html(trim_number(b))
}

function token_bonus(a, b = "total") {
    var c, d = a ? parseFloat(a) : 0,
        e = 0,
        f = b ? b : "total",
        g = 0,
        h = base_bonus ? parseFloat(base_bonus) : 0,
        i = amount_bonus ? amount_bonus : {
            1: 0
        };
...
function token_calc(a) {
    var b = $(a),
        ...
        E = 0,
        F = 0;
    console.log($.number(w * y, decimals.max, ".", ""));
    ...
    0 === z ? (token_alert(k, "info-circle", "icon"), l.addClass("disabled").removeAttr("data-amount")) : z >= minimum_token && z <= maximum_token ? (token_alert(k, "check-circle text-success", "icon"), token_alert(k, A * minimum_token, "amount"), token_alert(k, "", "text"), l.removeClass("disabled")) : z < minimum_token ? (token_alert(k, "times-circle text-danger", "icon"), l.addClass("disabled").removeAttr("data-amount")) : z > maximum_token && (token_alert(k, mstkmx, "text"), l.addClass("disabled").removeAttr("data-amount")), C = parseFloat(token_bonus(z, "base")), D = parseFloat(token_bonus(z, "amount")), E = parseFloat(token_bonus(z, "total")), F = parseFloat(z) + E, A = isNaN(A) ? 0 : A, F = isNaN(F) ? 0 : F;
    ...
    f.text(J), g.text(K), h.text(L), d.text(trim_number($.number(A, decimals.max, ".", ","))), i.text(M), p.text(trim_number($.number(A, decimals.max, ".", ","))), q.text(x), r.text(J), s.text(M), t.val(x), v.val(z), ("btc" == x || "ltc" == x || "eth" == x) && u.text("\"Coingate\""), ("usd" == x || "eur" == x || "gbp" == x) && u.text("\"Paypal\"");
    var N = amount_bonus ? amount_bonus : {
        1: 0
    };
...
function purchase_form_submit(a = $(".validate-form"), b = !0, c = "ti ti-info-alt") {
                ...
                success: function(e) {
                    ...
                    if (e.modal) {
                        var i = a.parents(".modal"),
                            j = !0;
                        is_changed = !0, i.modal("hide").addClass("hold"), i.find(".modal-content").html(e.modal), init_inside_modal(), i.on("hidden.bs.modal", function() {
    ...
    0 < a(".activity-delete").length && a(document).on("click", ".activity-delete", function() {
        swal({
            title: msasr,
            text: msdlg,
            icon: "warning",
            ...
            if (b) {
                ...
                a.post(e, {
                    ...
                    cl(b), "success" == b.msg && ("all" == c ? a("#activity-log tr").fadeOut(1e3, function() {
                        a(this).remove(), a("#activity-log").hide()
                    }) : a(".activity-delete").parents("tr.activity-" + c).fadeOut(1e3, function() {
                        a(this).remove()
                    }))
                }).fail(function(a, b, c) {
                    show_toast("error", mswng + "\n" + c), _log(a, b, c)
                })
            }
        })
    });
    ...
    if (0 < h.length) {
        ...
        if (0 < a(k).length) {
            ...
            l.on("sending", function(a, b, c) {
                ...
                0 < c.length && b.accepted && a.post(i, {
                    _token: csrf_token,
                    action: "delete",
                    file: c
                }).done(b => {
    ...
    a(".modal-close").on("click", function(b) {
        b.preventDefault(), !0 === s ? confirm(msodr) && (bs_modal_hide(a(this)), s = !1) : bs_modal_hide(a(this))
    });
    ...
    t.on("click", function(b) {
        ...
        var c = a(this),
            ...
            e = v.val(),
            f = w.val();
        e >= minimum_token && "" != f ? a.post(access_url, {
            _token: csrf_token,
            req_type: d,
            ...
            show_toast("error", mswng + "\n" + c), _log(a, b, c)
        }) : (e = v.val(), f = w.val(), show_toast("warning", mstkmn))
    });
    var x = a("a.user-wallet");
    x.on("click", function(c) {
        c.preventDefault(), 5 < user_wallet_address.length && a.post(user_wallet_address, {
            _token: csrf_token
        }).done(a => {
            cl(a), b.html(a), init_inside_modal(), 0 < b.children(".modal").length && b.children(".modal").modal("show")
        }).fail(function(a, b, c) {
            show_toast("error", mswng + "\n" + c), _log(a, b, c)
        })
    });
    a(document).on("click", "a.view-transaction", function(c) {
        ...
        a.post(view_transaction_url, {
            tnx_id: d,
            _token: csrf_token
        }).done(a => {
            cl(a), b.html(a), 0 < b.children(".modal").length && b.children(".modal").modal("show")
        }).fail(function(a, b, c) {
            show_toast("error", mswng + "\n" + c), _log(a, b, c)
        })
    });
    a(document).on("click", "a.user-modal-request", function(c) {
        ...
        "send-token" == e && "undefined" != typeof user_token_send ? d = user_token_send : "withdraw-token" == e && "undefined" != typeof user_token_withdraw && (d = user_token_withdraw), null !== d && f ? a.post(d, {
            type: f
        }).done(function(a) {
            ...
            show_toast("error", mswng + "\n" + c), _log(a, b, c)
        }) : show_toast("warning", msups, "ti ti-info-alt")
    });
    a(document).on("click", "a.user_tnx_trash", function(b) {
        b.preventDefault();
        var c = a(this).data("tnx_id"),
            d = a(this).attr("href");
        confirm(msasr) && a.post(d, {
            tnx_id: c,
            _token: csrf_token
        }).done(b => {
            cl(b), a("tr.tnx-item-" + c).fadeOut(400, function() {
                a(this).remove()
            }), cl(c), show_toast(b.msg, b.message, "ti ti-trash")
        }).fail(function(a, b, c) {
            show_toast("error", mswng + "\n" + c), _log(a, b, c)
        })
    })
