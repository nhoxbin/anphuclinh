...
class IcoHandler
{
    ...
    public static function panel_info($output = '')
    {
        ...
        $valid = gws('tokenlite_credible', Str::random(48));
        $return = $name;
    ...
    {
        // str_contains(app_key(), $this->find_the_path($this->getDomain()))
        // $this->cris_cros($this->getDomain(), app_key(2))
        return (!empty(env_file()) && str_contains(app_key(), gws('tokenlite_credible')) && true);
    }
    ...
    public static function get_manual_payment($type, $ext = '', $active = true)
    {
        ...
        if (in_array($type, ['usd', 'vnd', 'eur', 'gbp', 'cad', 'aud', 'try', 'rub', 'inr', 'brl', 'nzd', 'pln', 'jpy', 'myr', 'idr', 'ngn', 'mxn', 'php', 'chf', 'thb', 'sgd', 'czk', 'nok', 'zar', 'sek', 'kes', 'nad', 'dkk', 'hkd', 'huf', 'pkr', 'egp', 'clp', 'cop', 'jmd'])) {
    ...
    /* @function getCities()  @version v1.0 */
    public static function getCities()
    {
        $cities = Curl::to('https://provinces.open-api.vn/api/?depth=1')->asJson()->get();
        return $cities;
    }
