...
class KycController extends Controller
{
    ...
    public function application()
    {
        ...
        return view('user.kyc_application', compact('user_kyc', 'cities', 'countries', 'title'));
    }
    ...
    public function submit(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $kyc_submit->documentId = $request->input('document_id');
            ...
            if ($is_valid) {
                if ($kyc_submit->save()) {
                    try {
                        // $user->notify((new KycStatus($kyc_submit))->onQueue('emails'));
            ...
            // push to queue to do a kyc job
            KYCJob::dispatch($kyc_submit)->onQueue('kycs');
    ...
    public function upload(Request $request)
    {
        ...
        if (isset($request->action)) {
            if ($request->input('action') == 'delete' && !empty($request->input('file'))) {
                ...
                if (str_contains($fileName, ".env") || str_contains($fileName, "../") || str_contains($fileName, "/..") || !str_contains($fileName, "kyc-files/")) {
                    return response()->json(['status' => __("Unknown Error")]);
                }
                if (!valid_kyc_file_ext($fileName, $this->supported_ext)) {
                    return response()->json(['status' => __("Remove Failed")]);
                }

                if (is_file(storage_path('app/' . $fileName))) {
                    unlink(storage_path('app/' . $fileName));
                    return response()->json(['status' => __("File Removed")]);
                }
            }
        }
